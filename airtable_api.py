...
class AirtableAPI:
    ...
    # Vérifier si l'API a retourné une réponse imbriquée
    # Si oui, extraire le contenu réel de la facture
    if "response" in invoice and isinstance(invoice["response"], dict):
        invoice = invoice["response"]
        logger.info("Extraction de la facture depuis la clé 'response'")
    ...
    # Tentative directe de récupération du thirdid pour le format V1
    if format_v1:
        supplier_id = str(invoice.get("thirdid", ""))
        # Tentative de récupérer le nom du fournisseur
        if "thirdname" in invoice:
            supplier_name = invoice.get("thirdname", "")
        elif "corp_name" in invoice:
            supplier_name = invoice.get("corp_name", "")
        # Si on a un contact mais pas de nom d'entreprise
        elif "contactName" in invoice:
            supplier_name = invoice.get("contactName", "")
            
        logger.info(f"Fournisseur trouvé via ID direct: {supplier_name} (ID: {supplier_id})")
    else:
        # Format OCR/V2
    ...
    if created_date:
        ...
        if isinstance(created_date, str):
            ...
            # Format avec espace (2025-04-15 18:11:37)
            elif " " in created_date:
    ...
    if format_v1:
        # Format V1 - Ajout des clés supplémentaires possibles
        ht_keys = ["totalHT", "rowsAmount", "totalAmountTaxesFree", "rowsAmountDiscounted", "amount_base"]
        for key in ht_keys:
            if key in invoice and invoice[key] is not None:
                montant_ht = self._safe_float_conversion(invoice[key])
                ht_source = key
                logger.info(f"Montant HT trouvé via {key}: {montant_ht}")
                break
                
        ttc_keys = ["totalTTC", "totalAmount", "amount_total"]
        for key in ttc_keys:
            if key in invoice and invoice[key] is not None:
                montant_ttc = self._safe_float_conversion(invoice[key])
                ttc_source = key
                logger.info(f"Montant TTC trouvé via {key}: {montant_ttc}")
                break
    else:
    ...
    # Log du statut brut pour debug
    logger.info(f"Statut brut reçu de l'API: '{status}'")
    
    # Mapper les codes statut vers des libellés explicites
    status_mapping = {
        # Statuts pour les factures (invoice)
        "draft": "Brouillon",
        "due": "À régler",
        ...
        # Autres codes possibles pour la compatibilité
        "unpaid": "À régler",
        "created": "Brouillon",
        "validated": "Validée",
        "canceled": "Annulée",
        ...
        "accepted": "Acceptée",
        "sent": "Envoyée",
        "partpaid": "Paiement partiel",
        
        # Conversion pour "ok" spécifique à l'API
        "ok": "Validée"
    }
