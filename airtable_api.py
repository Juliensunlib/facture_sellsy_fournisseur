...
class AirtableSupplierAPI:
    ...
    def format_supplier_invoice_for_airtable(self, invoice):
        ...
        if not invoice:
            print("⚠️ Données de facture fournisseur invalides ou vides")
            return None
            
        # Affichage des clés principales pour débogage
        print(f"Structure de la facture fournisseur - Clés principales: {list(invoice.keys())}")
        
        # Récupérer l'ID fournisseur de Sellsy avec gestion des cas où les champs sont manquants
        supplier_id = None
        supplier_name = ""
        
        # Vérifier les différentes structures possibles de l'API Sellsy pour les informations fournisseur
        if "relation" in invoice:
            if "id" in invoice["relation"]:
                supplier_id = str(invoice["relation"]["id"])
            if "name" in invoice["relation"]:
                supplier_name = invoice["relation"]["name"]
            ...
            if not supplier_name:
                supplier_name = invoice.get("company_name", invoice.get("supplier_name", "Fournisseur #" + str(supplier_id) if supplier_id else ""))
        ...
        if created_date:
            ...
            print(f"⚠️ Date non trouvée pour la facture fournisseur {invoice.get('id', 'inconnue')}, utilisation de la date actuelle")
        ...
        result = {
            "ID_Facture_Fournisseur": str(invoice.get("id", "")),  # Conversion explicite en str
            "Numéro": reference,
            "Date": created_date,  # Date formatée correctement
            "Fournisseur": supplier_name,
            "ID_Fournisseur_Sellsy": supplier_id,  # Ajout de l'ID fournisseur Sellsy
    ...
    def find_supplier_invoice_by_id(self, sellsy_id):
        """Recherche une facture fournisseur dans Airtable par son ID Sellsy"""
        if not sellsy_id:
            print("⚠️ ID Sellsy vide, impossible de rechercher la facture fournisseur")
            return None
...
def sync_supplier_invoices_to_airtable(sellsy_api_client):
    print("🚀 Début de la synchronisation des factures fournisseur Sellsy vers Airtable...")

    # Récupère toutes les factures fournisseur depuis Sellsy
    invoices = sellsy_api_client.get_all_supplier_invoices()

    if invoices:
        print(f"📦 {len(invoices)} factures fournisseur récupérées depuis Sellsy.")
